...
function hermitInit() {
    ...
    for (var c = [], d = [], f = function(j) {
        if (a[j].dataset.songs) {
            d[j] = cloneObject(a[j].dataset),
            d[j].element = a[j],
            c[j] = new XMLHttpRequest,
            c[j].onreadystatechange = function() {
                var l = c.indexOf(this),
                m = d[l];
                if (m.storageName = "HxAP-Setting", 4 === this.readyState) if (200 <= this.status && 300 > this.status || 304 === this.status) {
                    var n = JSON.parse(this.responseText);
                    if (m.music = n.msg.songs, void 0 === m.music) return console.warn("Hermit-X failed to load " + d[l].songs),
                    !1;
                    void 0 === m.showlrc && (m.music[0].lrc ? m.lrcType = 3 : m.lrcType = 0),
                    1 === m.music.length && (m.music = m.music[0]),
                    m.autoplay && (m.autoplay = "true" === m.autoplay),
                    m.listfolded && (m.listFolded = "true" === m.listfolded),
                    m.mutex && (m.mutex = "true" === m.mutex),
                    m.narrow && (m.narrow = "true" === m.narrow),
                    ap[j] = new APlayer(m),
                    ap[j].parseRespons = n,
                    window.APlayerCall && window.APlayerCall[j] && window.APlayerCall[j](),
